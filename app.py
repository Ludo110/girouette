...
with nav_col4:
    if st.button("📹 Webcam Dinard", use_container_width=True):
...
# -----------------------------------------------------------------------------
# ONGLET 4 : WEBCAM PLAGE DE L'ÉCLUSE (DINARD)
# -----------------------------------------------------------------------------
elif st.session_state["onglet"] == "webcam":
    st.markdown(f"<div class='rect-style' style='padding:12px; text-align:center; max-width:580px; margin:15px auto 25px auto; color:#222;'><b>Webcam Plage de l'Écluse (Dinard)</b><br>Prochaine marée haute : {haute_mer} — Prochaine marée basse : {basse_mer}</div>", unsafe_allow_html=True)

    st.markdown("""
    <div class='rect-style' style='padding:20px; max-width:800px; margin:0 auto; text-align:center;'>
        <h3 style='color:#436e64; margin-top:0;'>📹 Plage de l'Écluse (Dinard) en direct</h3>
        ...
        <p style="margin-top:15px; font-size:0.9em; color:#444;">
            🌊 <i>Vue en direct sur la plage de l'Écluse à Dinard.</i>
        </p>
    </div>
    """, unsafe_allow_html=True)
